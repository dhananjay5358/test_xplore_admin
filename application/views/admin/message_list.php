...
<script type="text/javascript">			
	...
	{					
		var html = '<div class="box span12"><div class="box-header well" data-original-title><h2><i class="icon-edit"></i> Create New Welcome Message</h2></div><div class="box-content"><form method="POST" enctype="multipart/form-data" class="form-horizontal"><div class="control-group"><label class="control-label" for="add_image_name">Message * : </label><div class="controls"><textarea class="input-xlarge" type="text" id="add_message"></textarea></div></div></form></div></div>';
		var add_message_box=[{
						html: html,
	...
	{	
		$.ajax({
					url: '<?php echo site_url('admin/welcome_message/add_or_remove_message');?>',		
					type: 'POST',
					data: {
							'status' : 'add' ,
							'message': message
	...
	function delete_message(id)
	{
		...
		var show_delete_message_box=[{
					title:"Delete Image",
					html: "<strong>are you really want to delete this Welcome Message ?</strong>",
	...
	{
		$.ajax({
					url: '<?php echo site_url('admin/welcome_message/edit_message');?>',		
					type: 'POST',
					data: {
							'message' : message ,
							'id': id
			...
			<!-- content starts -->
				...
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-user"></i> Message List</h2>
						<div class="box-icon">
					...
					<div class="box-content">
						<button class="btn btn-info" style="float:right;" onClick="javascript:show_addMessage_box(); return false;"><i class="icon-edit icon-white"></i>Add Welcome Message</button>
						...
						<?php if($messages['0'] != "no-data") 
								{
									$i = (isset($index) ? $index : 0);
									foreach($messages as $message)
									{ $i++;?>
										<tr id="messagetr_<?php echo $message['message_id'];?>">
											<td><?php echo $i;?></td>
											<td id="messageName_<?php echo $message['message_id'];?>"><?php echo $message['message'];?></td>
											<td>
												<a class="btn btn-info"  id="edit_<?php echo $message['message_id'];?>" href="#" onClick="javascript:show_editMessage_box(this.id); return false;"><i class="icon-edit icon-white"></i>Edit</a>																								
												<a class="btn btn-danger"  id="delete_<?php echo $message['message_id'];?>" href="#" onClick="javascript:delete_message(this.id); return false;"><i class="icon-trash icon-white"></i>Delete</a>
											</td>								
										</tr>
						  <?php 	}
								} ?>
