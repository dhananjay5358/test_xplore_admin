...
if(!empty($curl_response))
{
	foreach ($curl_response->response->responses as $key => $response_arr) {
		...
		if(!empty($response))
		{
			...
			$detail['type'] = isset($old_detail->type) ? str_replace("'", "", $old_detail->type) : "";
			$detail['Phone'] = $new_phone;
			$detail['Likes'] = $like_count;
			$detail['Website'] = isset($old_detail->Website) ? str_replace("'", "", $old_detail->Website) : '';
			$detail['Path'] = isset($old_detail->Path) ? str_replace("'", "", $old_detail->Path) : "";
			$detail['profileimage'] = isset($old_detail->profileimage) ? str_replace("'", "", $old_detail->profileimage) : "";
